...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_user_missions_ends_at_status", "user_missions", ["ends_at", "status"]
    )

    # add unique constraint to mission_characters (user_mission_id, character_id)
    op.create_unique_constraint(
        "uq_mission_char_user_mission_character",
        "mission_characters",
        ["user_mission_id", "character_id"],
    )


def downgrade() -> None:
    op.drop_constraint(
        "uq_mission_char_user_mission_character", "mission_characters", type_="unique"
    )
